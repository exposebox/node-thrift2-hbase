...
function fetchSync(table, key, columns, options, callback) {
...
const fetch = Promise.promisify(fetchSync);
...
Service.prototype.getRow = function (table, key, columns, options, callback) {
    ...
    cache.getRow(table, key, columns, options)
        .then(callback)
        .catch(function () {
            debug('couldnt find in table', table, 'value for key', key,
                'with columns', columns, 'fetching!');
            fetch(table, key, columns, options)
                .then(function (data) {
                    debug('fetched data', data);
                    cache.setRow(table, key, columns, options, data);
                    callback(data);
                })
        });
};
